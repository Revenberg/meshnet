...
echo Flashing COM11...
echo [1/2] Erasing...
"%ESPTOOL%" --chip esp32s3 --port COM11 --before default-reset --after hard-reset erase-flash

echo.
echo [2/2] Writing V0.8.1...
"%ESPTOOL%" --chip esp32s3 --port COM11 --before default-reset --after hard-reset write-flash -z --flash-mode dio --flash-freq 80m --flash-size 8MB ^
    0x0 "%BOOTLOADER%" ^
    0x8000 "%PARTITIONS%" ^
    0x10000 "%BINARY%"
